...
def parse_nutrition_lines(text: str) -> List[Dict[str, Optional[str]]]:
    ...
    """
    # 중앙 유틸 사용 (일반 파이프라인 + '96'→'%' 등 nutrition 보정 포함)
...
def parse_nutrition_easyocr(pil, lang: str):
    ...
    - 이름 토큰 발견 후 근접 N토큰 범위에서 수치/단위/퍼센트 탐색
    - 컷오프/윈도우 크기는 rules_data에서 통일 관리
    """
    results = run_ocr_easyocr_with_boxes(pil, lang)
    tokens = [(t.strip(), c) for (_b, t, c) in results if t and c >= EASYOCR_CONFIDENCE_MIN]

    # 서버 후처리 적용본(검증용 텍스트) — 중앙 유틸 사용
    text_pp = nutrition_normalize(" ".join([t for t, _ in tokens]))
    ...
    for t, c in tokens:
        # 숫자 토큰의 흔한 오인식만 nutrition 규칙으로 치환
        if _token_has_digit(t):
            t = nutrition_normalize(t)
